...
def user_similarity(user_feat,item_user_bhvs,need_tile=True):
    U = item_user_bhvs.get_shape().as_list()[1]
    user_feats = tf.tile(user_feat, [1, U,1]) if need_tile else user_feat
    ...
    score = tf.divide(pooled_mul_12, pooled_len_1 * pooled_len_2 + 1e-8, name="scores")
    return score