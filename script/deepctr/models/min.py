...
def MIN(dnn_feature_columns, history_feature_list, itemshort=False, transformer_num=1, att_head_num=8,
    ...
    user_hist_emb_list = embedding_lookup(embedding_dict, features, user_history_feature_columns,
                                 return_feat_list=user_history_fc_names, to_list=True)
    ...
    hist = AttentionSequencePoolingLayer2(att_hidden_units=(80, 40), att_activation="dice", mask_reverse=itemshort,
                                         weight_normalization=False, supports_masking=False)([
    ...
    for _ in range(transformer_num):
        ...
        for i in range(U):
            ...
            transformer_output_tmp = transformer_layer([transformer_outputs[:,i,:,:], transformer_outputs[:,i,:,:],
                                                    userlist_behavior_length[:,i,:], userlist_behavior_length[:,i,:]])
