...
#针对多用户的序列行为创建函数
class VarUserLenSparseFeat(namedtuple('VarUserLenSparseFeat',
                                  ['sparsefeat', 'maxuser', 'maxlen', 'combiner', 'user_length_name','length_name', 'weight_name', 'weight_norm'])):
    __slots__ = ()

    def __new__(cls, sparsefeat, maxuser, maxlen, combiner="mean", user_length_name=None,length_name=None, weight_name=None, weight_norm=True):
        return super(VarUserLenSparseFeat, cls).__new__(cls, sparsefeat, maxuser, maxlen, combiner, user_length_name, length_name, weight_name,
                                                    weight_norm)
...
def build_input_features(feature_columns, prefix=''):
    ...
    for fc in feature_columns:
        ...
        elif isinstance(fc, VarUserLenSparseFeat):
            input_features[fc.name] = Input(shape=(fc.maxuser,fc.maxlen,), name=prefix + fc.name,
                                            dtype=fc.dtype)
            if fc.weight_name is not None:
                input_features[fc.weight_name] = Input(shape=(fc.maxuser,fc.maxlen, 1), name=prefix + fc.weight_name,
                                                       dtype="float32")
            if fc.length_name is not None:
                input_features[fc.length_name] = Input((fc.maxuser,1,), name=prefix + fc.length_name, dtype='int32')
                
            if fc.user_length_name is not None:
                input_features[fc.user_length_name] = Input((1,), name=prefix + fc.user_length_name, dtype='int32')
